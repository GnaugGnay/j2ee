...
class D{
	...
	public static function init(){
		...
		//加载器
		spl_autoload_register('D::loader');

		//设置语言
		Lang::setLanguage();

		//全局code
		require ROOT . '/config/code.php';
	...
	public static function loader($className){
		...
		$file = ''; 
		if(in_array($className, array('Controller', 'Debug', 'Model', 'Service', 'Lang'))){//框架本身
		...
		if($file && is_file($file)){
			...
			foreach(D::$includePaths as $path){
	...
	/*
	 * 专门用于跨app加载service
	 * 调用方式为  D::call('/app/service/method', $arg1, $arg2)
	 */
	public static function call($methodPath){
		...
		return self::callArray($methodPath, $args);
	}
	 ...
	 * 调用方式为  D::call('/app/service/method', $arg1, $arg2)
	 */
	public static function callArray($methodPath){
		$methodPath = explode('/', $methodPath);
		$method = array_pop($methodPath);
		$service = array_pop($methodPath);

		D::$thisAppRoot = ROOT . '/web' . implode('/', $methodPath);
		...
		$serviceFile = D::$thisAppRoot . '/' . SRV_DIR . '/' . $service . '.php';
...
function import($path){
	array_unshift(D::$includePaths, $path);
}
