...
class LibFile{
	...
	public static function uploadByFile($name,$tmp_name,$size, $uploadPath, $uploadUrl, $size = 1000, $ext = array(), $fileName = ''){
		$res = array();
		$tmp_name = str_replace("\\\\", "\\", $tmp_name); //框架自动加反斜杠了
		if(!is_uploaded_file($tmp_name) || !$name){
			$res['state'] = false;
			$res['url'] = "";
			$res['msg'] = "UPLOAD_ERROR_FILE";
			return $res;
		}

		$uploadInfo = self::getUploadInfo($name, $uploadPath, $uploadUrl, $fileName);
